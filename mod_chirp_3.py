...
from chirp_final import repet_val,valor_n
...
def chirpStandard(sr_rx,A,fs_Hz,rep_Hz,f0_Hz,f1_Hz,phase_rad=0,mode=0):      		##rep_Hz inversa De la duracion del CHIRP
    ...
    #print("n: ",n)
    n_rep = int(T_rep*sr_rx)			# Número de repeticiones
    ...
    print(len(freq_Hz))
    print("-------------------------------------")
    freq_Hz = valor_n(freq_Hz,n_rep-1)
    freq_Hz = freq_Hz[:-1]
    print(len(t_s))
    print(len(freq_Hz))
    phi_Hz  = freq_Hz*t_s  			# PHI INSTANTENEA               	//f(t)*t -> F(t)
    print(len(phi_Hz))
    phi_Hz = valor_n(phi_Hz,n_rep-1)
    phi_Hz = repet_val(phi_Hz,n_rep)
    #print(phi_Hz)
    ...
    return  t_s,A*np.exp([j * 1j for j in phi_rad]),T_rep,n
...
#a = chirp_usrp(2500000,1,0.0004,15,20000000,0,4000000)
#print(a)
#print(len(a))
